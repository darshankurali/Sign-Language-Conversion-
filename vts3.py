    ...
    def process_audio():
        ...
        try:
            with sr.Microphone() as source:
                ...
                matched_phrases = []
                all_results = []  # To track all words/phrases and their status
                i = 0
                while i < len(phrases):
                    matched = False
                    for j in range(len(phrases), i, -1):
                        ...
                        if gif_path:
                            ...
                            all_results.append((combined_phrase, True))  # Mark as matched
                            i = j
                            matched = True
                            break
                    if not matched:
                        all_results.append((phrases[i], False))  # Mark as unmatched
                        i += 1

                # Display matched GIFs or show feedback for missing ones
                if matched_phrases:
                    display_gifs_in_batches(matched_phrases)
                for phrase, matched in all_results:
                    update_history(phrase, gif_available=matched)
